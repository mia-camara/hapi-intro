exports.register = function(server, options, next) {
  ...
  // place all routes for the API
  server.route([
    { // Hello World
      method: 'GET',
      path: '/',
      handler: function(request, reply) {
      reply("Hello, Im an awesome HarryQuote API Server");
    }
   },
    { // GET ALL THE QUOTES
      method: 'GET',
...
//         reply(request.params);
        if (Number.isNaN(id)) {
          return reply("Comeon Bro, give me a number").code(404);
        }
      reply( db[id] );
      }
    },
     { // POST --> Adds a new quote
      method: 'POST',
      path: '/quotes',
      handler: function(request, reply)  {
        var newQuote = (request.payload);
        db.push(newQuote);
